...
const MissionHealthLoadingScreen: React.FC = () => {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gray-50 text-gray-800 p-4">
      <div 
        className="w-80 h-80 md:w-96 md:h-96 bg-white rounded-lg flex flex-col items-center justify-center p-8 relative shadow-lg border border-gray-200"
        style={{ 
      ...
      >
        ...
        >
          <p className="text-xl md:text-2xl font-serif mb-4 leading-relaxed text-gray-700">
            {randomQuote.quote}
          </p>
        ...
        <div className="absolute bottom-10 flex space-x-2">
          {[0, 1, 2].map((dot) => (
            <div 
              key={dot}
              className={`w-3 h-3 rounded-full ${dot === currentDot ? 'bg-gray-700' : 'bg-gray-300'}`}
            />
          ))}
        </div>
      </div>
